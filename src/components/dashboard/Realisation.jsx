...
export default function Realisation() {
  ...
  const edit=async(id)=>
  {
      try {
        if (id) {
          navigate('/dashboard/المنجزات/'+id)
        } else {
  ...
  const remeveFinal=async(id)=>
  {
    try {
      if(id)
      {
          ...
          await setMsg('لقد ثم حدف منجز بنجاح');
          await getData();
          await handleClick();
      }
      else
  ...
  return (
    <Box>
      {data.length!==0?<Paper sx={{ width: '80%', overflow: 'hidden',m:'auto' }}>
        <Typography variant="h5" align='right' sx={{marginTop:'3%',marginRight:'3%'}}><b>:المنجزات</b></Typography>
        <Stack sx={{ width:'95%',display:'flex',flexDirection:'row',justifyContent:'end',m:'auto' }}>
        <Button
      onClick={()=>navigate('/dashboard/المنجزات/إضافة')}
      variant="contained"
      sx={{
              ...
              .map((row,index) => {
                return (
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.code} >
                    ...
                    <TableCell  align='right'>
                    {console.log(row.img)}
                      <img src={row.img} width='90px' height='70px'/>
                    </TableCell>
                    <TableCell  align='right'>
                      <Stack direction="row" >
      ...
      >
        ...
        <DialogContent dir='rtl'>
          <DialogContentText sx={{ display:'flex',justifyContent:'center' }}>
           <img src={oneData.img} width='90px' height='60px'/>
          </DialogContentText>
          <DialogContentText>
           {oneData.data}
          </DialogContentText>
