...
export default function VisionAdd() {
    ...
    const ajouter=async()=>
    {
        try {
            ...
            setMsg('لقد ثم إضافة الرؤيا بنجاح');
            setEtatAlert('success');
            setOpenAlert(true);
            setTimeout(() => {
                setOpenAlert(false);
                navigate('/dashboard/الرؤيا');
            }, 2000);
        } catch (error) {
            setMsg('error');
            setEtatAlert('error');
  ...
  return (
    ...
    <Paper sx={{ width: '80%', overflow: 'hidden',m:'auto' }}>
      <Typography variant="h5" align='right' sx={{marginTop:'3%',marginRight:'3%'}} dir='rtl'><b>إضافة الرؤيا :</b></Typography>
      <Stack>
      <TextField
      ...
      <Button onClick={()=>navigate('/dashboard/الرؤيا')} >رجوع</Button>
      </Stack>
      </Paper>
      {/* snackbar  */}
