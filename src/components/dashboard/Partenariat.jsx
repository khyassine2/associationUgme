...
export default function Partenariat() {
  ...
  const edit=async(id)=>
  {
      try {
        if (id) {
          navigate('/dashboard/الشراكات/'+id)
        } else {
  ...
  const remeveFinal=async(id)=>
  {
    try {
      if(id)
      {
          ...
          await setMsg('لقد ثم حدف الشراكة بنجاح');
          await getData();
          await handleClick();
      }
      else
  ...
  return (
    ...
    {data.length!==0?<Paper sx={{ width: '80%', overflow: 'hidden',m:'auto' }}>
      <Typography variant="h5" align='right' sx={{marginTop:'3%',marginRight:'3%'}}><b>:الشراكات</b></Typography>
      <Stack sx={{ width:'95%',display:'flex',flexDirection:'row',justifyContent:'end',m:'auto' }}>
      <Button
    onClick={()=>navigate('/dashboard/الشراكات/إضافة')}
    variant="contained"
    sx={{
              ...
              return (
                <TableRow hover role="checkbox" tabIndex={-1} key={row.code} >
                  ...
                  <TableCell  align='center'>
                    <img src={row.img} width='100px' height='70px'/>
                  </TableCell>
                  <TableCell  align='center'>
                    <Stack direction="row" >
  ...
  <Dialog
    ...
    >
      ...
      <DialogContentText id="alert-dialog-description" dir='rtl'>
        ...
        <DialogContentText sx={{ display:'flex',justifyContent:'center' }}>
         <img src={oneData.img} width='300px' height='250px'/>
        </DialogContentText>
      </DialogContent>
      <DialogActions>
    ...
    >
      <DialogTitle id="alert-dialog-title">
        {"هل أنت متأكد من حذف هذا الشراكة ؟"}
      </DialogTitle>
      <DialogContent>
        <DialogContentText id="alert-dialog-description" dir='rtl'>
        <b>الشراكة مع:</b>
