...
export default function Strategie() {
  ...
  const edit=async(id)=>
  {
      try {
        if (id) {
          navigate('/dashboard/الاستراتيجية/'+id)
        } else {
  ...
  const remeveFinal=async(id)=>
  {
    try {
      if(id)
      {
          ...
          await setMsg('لقد ثم حدف الاستراتيجية بنجاح');
          await getData();
          await handleClick();
      }
      else
  ...
  return (
    <Box>
      {data.length!==0?<Paper sx={{ width: '80%', overflow: 'hidden',m:'auto' }}>
        <Typography variant="h5" align='right' sx={{marginTop:'3%',marginRight:'3%'}}><b>:الاستراتيجية</b></Typography>
        <Stack sx={{ width:'95%',display:'flex',flexDirection:'row',justifyContent:'end',m:'auto' }}>
        <Button
      onClick={()=>navigate('/dashboard/الاستراتيجية/إضافة')}
      variant="contained"
      sx={{
    ...
    <Dialog
      ...
      >
        ...
        <DialogContent dir='rtl'>
          <DialogContentText>
          <b>الاستراتيجية هي:</b><br/>
          <b>{oneData.title}: </b> {oneData.data}
          </DialogContentText>
        </DialogContent>
        <DialogActions>
      ...
      >
        <DialogTitle id="alert-dialog-title">
          {"هل أنت متأكد من حذف هذه الاستراتيجية؟"}
        </DialogTitle>
        <DialogContent>
