...
export default function StrategieAdd() {
    ...
    const ajouter=async()=>
    {
        try {
            ...
            const dataFinal = { 'title':title.current.value,'data': data.current.value };
            // Utilisez la fonction addDoc pour ajouter un nouveau document
            await addDoc(collectionRef, dataFinal);
            setMsg('لقد ثم إضافة الاستراتيجية بنجاح');
  ...
  return (
    ...
    <Paper sx={{ width: '80%', overflow: 'hidden',m:'auto' }}>
      <Typography variant="h5" align='right' sx={{marginTop:'3%',marginRight:'3%'}} dir='rtl'><b>إضافة الاستراتيجية :</b></Typography>
      <Stack>
            <Tooltip title='يرجى كتابة عنوان الاستراتيجية'>
                <TextField
                id="outlined-multiline-static"
                multiline
                rows={1}
                inputRef={title}
                placeholder='يرجى كتابة عنوان الاستراتيجية'
                sx={{ width:'75%',m:'auto',mt:'2% !important' }}
                dir='rtl'
                />
            </Tooltip>
            <Tooltip title='يرجى كتابة الاستراتيجية'>
                <TextField
                id="outlined-multiline-static"
                multiline
                rows={4}
                inputRef={data}
                placeholder='يرجى كتابة الاستراتيجية'
                sx={{ width:'75%',m:'auto',mt:'2% !important' }}
                dir='rtl'
                />
            </Tooltip>
